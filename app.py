...
from urllib.parse import quote_plus # <-- TAMBAHAN BARU
...
# >>>>> FUNGSI SCRAPING YANG SUDAH DIPERBARUI <<<<<
def scrape_data_from_web(pasaran, total):
    """
    Fungsi BARU untuk mengambil data dari server.scanangka.fun.
    """
    # Memformat nama pasaran agar aman untuk URL (misal: "NEW YORK MIDDAY" -> "new+york+midday")
    formatted_pasaran = quote_plus(pasaran.lower())
    base_url = f"https://server.scanangka.fun/keluaran/pasaran/{formatted_pasaran}"
    
    st.info(f"Mencoba scraping dari: {base_url}")
    ...
    try:
        # Menambahkan .text untuk memastikan pandas membaca konten HTML mentah
        html_content = requests.get(base_url, headers=headers).text
        list_of_tables = pd.read_html(html_content)

        if not list_of_tables:
            st.error(f"Tidak ada tabel data yang ditemukan di halaman untuk pasaran '{pasaran}'.")
            return []

        # Asumsikan data yang relevan ada di tabel pertama
        df = list_of_tables[0]

        if df.shape[1] < 3:
            st.error(f"Struktur tabel tidak terduga. Ditemukan {df.shape[1]} kolom, diharapkan setidaknya 3.")
            return []

        # Asumsikan angka keluaran selalu ada di kolom TERAKHIR.
        # .iloc[:, -1] memilih semua baris dari kolom terakhir
        angka_list = pd.to_numeric(df.iloc[:, -1], errors='coerce').dropna().astype(int).astype(str).str.zfill(4)

        if angka_list.empty:
            st.warning("Kolom terakhir tidak berisi data angka yang valid.")
            return []

        # Ambil data sebanyak 'total' dari atas (data terbaru) dan urutkan kembali
        # Website menampilkan data terbaru di atas, jadi kita ambil head(total)
        # lalu balik urutannya agar data terlama ada di awal (sesuai format model)
        return angka_list.head(total).iloc[::-1].tolist()

    except Exception as e:
        st.error(f"Terjadi kesalahan saat proses scraping: {e}")
        return []

# --- Sisa fungsi-fungsi inti Anda (tidak ada perubahan) ---
...
def find_best_window_size(df, label, model_type, min_ws, max_ws, top_n, top_n_shio):
    ...
    if is_jalur_scan:
        problem_type = "jalur_multiclass"; k_val = 2; num_classes = 3; table_cols = ["Window Size", "Prediksi", "Angka Jalur"]
    elif label in BBFS_LABELS:
        problem_type = "multilabel"; k_val = top_n; num_classes = 10; table_cols = ["Window Size", f"Top-{k_val}"]
    elif label in SHIO_LABELS:
        problem_type = "shio"; k_val = top_n_shio; num_classes = 12; table_cols = ["Window Size", f"Top-{k_val}"]
    else:
        problem_type = "multiclass"; k_val = top_n; num_classes = 10; table_cols = ["Window Size", f"Top-{k_val}"]
    ...
    for i, ws in enumerate(range(min_ws, max_ws + 1)):
        ...
        try:
            ...
            if is_jalur_scan:
                acc = eval_results[1]; top_2_acc = eval_results[2]
                last_pred = model.predict(X[-1:], verbose=0)[0]; top_indices = np.argsort(last_pred)[::-1][:2]; pred_str = f"{top_indices[0] + 1}-{top_indices[1] + 1}"
                top_jalur_num = top_indices[0] + 1; second_jalur_num = top_indices[1] + 1
                ...
                preds = model.predict(X_val, verbose=0); avg_conf = np.mean(np.sort(preds, axis=1)[:, -k_val:]) * 100
                last_pred = model.predict(X[-1:], verbose=0)[0]; top_indices = np.argsort(last_pred)[::-1][:k_val]
                if problem_type == "shio": top_n_pred_str = ", ".join(map(str, top_indices + 1))
                else: top_n_pred_str = ", ".join(map(str, top_indices))
                if problem_type == 'multilabel':
                    acc = eval_results[1]; score = (acc * 0.7) + (avg_conf / 100 * 0.3)
                else:
                    acc = eval_results[1]; top_n_acc = eval_results[2]; score = (acc * 0.2) + (top_n_acc * 0.5) + (avg_conf / 100 * 0.3)
...
# Gunakan daftar pasaran yang lebih lengkap sesuai situs target
pasaran_list = [
    "NEW YORK MIDDAY", "MOROCCO QUATRO 02:00", "NORTH CAROLINA DAY", "DARWIN", "ORLANDO",
    "SYDNEY", "HONGKONG", "SINGAPORE", "BULLSEYE", "GERMANY PLUS5"
]
...
with st.sidebar:
    ...
    selected_lokasi = st.selectbox("🌍 Pilih Pasaran", pasaran_list) # <-- Menggunakan daftar pasaran baru
...
with col1:
    # =======================================================================
    # >>>>> BLOK PENGAMBILAN DATA MENGGUNAKAN FUNGSI SCRAPING BARU <<<<<
    # =======================================================================
    if st.button("Ambil Data dari Keluaran Angka", use_container_width=True):
        # selected_lokasi sudah dipilih dari dropdown di sidebar
        with st.spinner(f"Memulai scraping data untuk {selected_lokasi}..."):
            scraped_angka = scrape_data_from_web(selected_lokasi, putaran)

        if scraped_angka:
            st.session_state.angka_list = scraped_angka
            st.success(f"{len(scraped_angka)} data terakhir untuk {selected_lokasi} berhasil di-scrape.")
            st.rerun()
        else:
            st.warning("Gagal mendapatkan data dari web scraping. Coba periksa error di atas.")


with col2: st.caption("Data angka dari web scraping akan digunakan untuk pelatihan dan prediksi.")
...
# --- Sisa kode UI (manajemen, scan, angka main) tidak ada perubahan ---
with tab_manajemen:
        ...
        with st.expander(expander_title, expanded=True):
            if data == "PENDING":
                ...
                if result_df is not None and not result_df.empty:
                    ...
                    if 'Angka Jalur' in result_df.columns: styler = styler.set_properties(subset=['Angka Jalur'], **{'white-space': 'pre-wrap', 'text-align': 'left'})
                    st.dataframe(styler)
                    st.markdown("---")
    ...
    if len(df) < 10:
        ...
        with col1:
            ...
            with st.expander("Analisis AI Depan (berdasarkan digit EKOR/Satuan)", expanded=True):
                with st.spinner("Menganalisis AI Depan..."): result_depan = calculate_markov_ai(df, top_n=jumlah_digit, mode='depan')
            ...
            with st.expander("Analisis AI Tengah (berdasarkan digit AS/Ratusan)"):
                with st.spinner("Menganalisis AI Tengah..."): result_tengah = calculate_markov_ai(df, top_n=jumlah_digit, mode='tengah')
            ...
            with st.expander("Analisis AI Belakang (berdasarkan digit KOP/Ribuan)"):
                with st.spinner("Menganalisis AI Belakang..."): result_belakang = calculate_markov_ai(df, top_n=jumlah_digit, mode='belakang')
        ...
        with col2:
            ...
            with st.spinner("Menghitung statistik..."): stats = calculate_angka_main_stats(df, top_n=jumlah_digit)
